"""Stress tests: binary search for minimum polling interval on L6."""
...
from linkerbot.hand.l6 import AngleEvent, ForceSensorEvent, SensorSource

pytestmark = [pytest.mark.l6, pytest.mark.stress]
...
TORQUE_READ_INTERVAL_MIN_SEC = 0.05  # Minimum random gap between torque reads
TORQUE_READ_INTERVAL_MAX_SEC = 0.20  # Maximum random gap between torque reads
TORQUE_READ_TIMEOUT_MS = 300  # Blocking timeout per torque read attempt
...
class TestPollingIntervalStress:
    ...
    def test_angle_interval(self, l6_hand: L6):
    ...
    def test_force_sensor_interval(self, l6_hand: L6):
    ...
    def test_combined_interval(self, l6_hand: L6):
...
class TestInterleavedReadDuringPolling:
    ...
    interleaves blocking torque reads to verify no frame collisions.
    L6 speed is write-only; torque supports get_blocking().
    """

    def test_torque_read_during_polling(self, l6_hand: L6):
        """Randomly issue torque.get_blocking() while default polling is running.

        Default polling (ANGLE@60Hz, FORCE_SENSOR@30Hz) is auto-started.
        This test sends blocking torque reads at random intervals and checks
        ...
        print(f"\n  Running interleaved torque reads for {INTERLEAVE_WINDOW_SEC}s ...")
        print(
            f"  Polling: ANGLE@60Hz + FORCE_SENSOR@30Hz  |  "
            f"Torque read timeout: {TORQUE_READ_TIMEOUT_MS}ms"
        )

        while time.monotonic() < deadline:
            gap = random.uniform(
                TORQUE_READ_INTERVAL_MIN_SEC, TORQUE_READ_INTERVAL_MAX_SEC
            )
            time.sleep(gap)
            ...
            t0 = time.monotonic()
            try:
                hand.torque.get_blocking(timeout_ms=TORQUE_READ_TIMEOUT_MS)
        ...
        print(f"\n  {'=' * 56}")
        print("  Interleaved Torque Read — Results")
        print(f"  {'=' * 56}")
        ...
        assert total_sent > 0, "No torque reads were sent"
        assert success_rate >= SUCCESS_RATIO, (
            f"Torque read success rate {success_rate:.1%} < {SUCCESS_RATIO:.0%}: "
