"""Tests for A7 arm complex motion with hardware (interactive).
...
from linkerbot.arm import A7, Pose
from tests.conftest import InteractiveSession

pytestmark = [pytest.mark.a7, pytest.mark.interactive, pytest.mark.motion]
...
MAESTRO_INIT_POSE: dict[str, list[float]] = {
    "left": [0.0, 0.30, -0.25, 1.85, 0.0, -1.57],
    "right": [0.0, 0.30, -0.25, 1.85, 0.0, 1.57],
}
...
MOVE_L_PARAMS: dict[str, dict[str, Any]] = {
    "default": {
        "max_velocity": 0.05,
        "acceleration": 0.1,
...
MAESTRO_URDF_OFFSET: dict[str, list[float]] = {
    "left": [0.0328, 0.171, 0.0],
    "right": [0.0328, -0.171, 0.0],
}

CONSISTENCY_PAIRS: dict[str, list[dict[str, list[float]]]] = {
    "left": [
        {
            "maestro": [0.0, 0.33, -0.27, 1.5, 0.0, -1.57],
            "urdf": [0.3628, 0.171, -0.27, 0.0, -1.5, -1.57],
        },
        {
            "maestro": [0.0, 0.33, -0.27, 1.5, 0.0, 0.0],
            "urdf": [0.3628, 0.171, -0.27, 0.0, -1.5, 0.0],
        },
    ],
    "right": [
        {
...
URDF_INIT_POSE: dict[str, list[float]] = {
    "left": [0.3628, 0.171, -0.27, 0.0, -1.5, -1.57],
    "right": [0.3628, -0.171, -0.27, 0.0, -1.5, 1.57],
}
...
class TestMoveJBasic:
    ...
    def test_move_j_profiles(
        ...
        session.step(
            ...
            action=lambda: (a7_arm.enable(), a7_arm.home(blocking=True)),
        ...
        for prof_label, vel, acc in MOVE_J_PROFILES:
            session.step(
                ...
                action=lambda v=vel, a=acc: (
                    a7_arm.set_velocities([v] * NUM_JOINTS),
                    a7_arm.set_accelerations([a] * NUM_JOINTS),
                    time.sleep(0.5),
                ),
...
class TestMovePMaestro:
    ...
    def test_move_p_maestro_frame(
        ...
        session.step(
            ...
            action=lambda: a7_arm.move_p(init_pose_obj, blocking=True),
        ...
        for idx, (label, offset) in enumerate(CARTESIAN_CASES_DEFAULT, 1):
            ...
            session.step(
                ...
                action=lambda t=target: a7_arm.move_p(t, blocking=True),
        ...
        session.step(
            instruction="Returning HOME, then disable",
            action=lambda: (a7_arm.home(blocking=True), a7_arm.disable()),
    ...
    def test_move_l_maestro_frame(
        ...
        for idx, (label, offset) in enumerate(CARTESIAN_CASES_DEFAULT, 1):
            ...
            session.step(
                ...
                action=lambda t=target, p=default_params: a7_arm.move_l(t, **p),
                ...
                action=lambda p=default_params: a7_arm.move_l(init_pose_obj, **p),
                expected="TCP back at initial pose",
            )
        ...
        for idx, (label, offset) in enumerate(CARTESIAN_CASES_HIGH, 10):
            ...
            session.step(
                ...
                action=lambda t=target, p=high_params: a7_arm.move_l(t, **p),
                expected=f"TCP should reach target at high speed ({label})",
            ).step(
                instruction=f"[HIGH #{idx}] Returning to initial pose via move_l",
                action=lambda p=high_params: a7_arm.move_l(init_pose_obj, **p),
                expected="TCP back at initial pose",
            )
        ...
        for idx, (label, offset) in enumerate(CARTESIAN_CASES_LOW, 12):
            ...
            session.step(
                ...
                action=lambda t=target, p=low_params: a7_arm.move_l(t, **p),
                expected=f"TCP should reach target at low speed ({label})",
            ).step(
                instruction=f"[LOW #{idx}] Returning to initial pose via move_l",
                action=lambda p=low_params: a7_arm.move_l(init_pose_obj, **p),
                expected="TCP back at initial pose",
            )
...
class TestCoordinateConsistency:
    ...
    def test_maestro_urdf_consistency(
        ...
        session.step(
            ...
            action=lambda: (
                a7_arm.enable(),
                a7_arm.set_velocities([1.0] * NUM_JOINTS),
                a7_arm.set_accelerations([10.0] * NUM_JOINTS),
                time.sleep(0.5),
                a7_arm.home(blocking=True),
            ),
        ...
        for pair_idx, pair in enumerate(pairs):
            ...
            def _move_and_verify(
                ...
                a7_arm.move_p(t, blocking=True)

                actual = a7_arm.get_pose().to_list()
...
#
# These tests create a separate A7 instance with world_frame="urdf".
# Run them separately from the Maestro-frame tests to avoid having two
# instances of A7 send commands to the same CAN bus simultaneously:
#   pytest tests/arm/a7/test_complex_motion.py -k TestCartesianURDF
# ===================================================================
...
class TestCartesianURDF:
    ...
    Creates its own A7 instance with world_frame='urdf' and manages
    the full lifecycle internally via try/finally.
    """

    @staticmethod
    def _create_urdf_arm(side: str) -> A7:
