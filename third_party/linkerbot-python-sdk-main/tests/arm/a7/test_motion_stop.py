"""Tests for A7 arm motion stop and PID display (interactive)."""
...
from tests.arm.a7.conftest import prepare_interactive_test
...
class TestMotionStop:
    ...
    def test_motion_detect_and_emergency_stop(
        ...
        a7_arm: A7,
        arm_side: str,
        a7_pid_params: dict[str, list[float]],
        ...
        print("\nCurrent motor PID parameters (from a7_arm fixture):")
        for name, values in a7_pid_params.items():
            print(f"  {name}: {values}")
        ...
        session.step(
            ...
            action=lambda: prepare_interactive_test(a7_arm),
        ...
        def _move_and_check() -> None:
            ...
            a7_arm.move_j(target, blocking=False)

            time.sleep(0.3)
            moving = a7_arm.is_moving()
            ...
            time.sleep(0.5)
            a7_arm.emergency_stop()
            print("emergency_stop() sent")

        session.step(
            instruction=(
                "Non-blocking move_j: joint 2 -> +1.0 rad. "
