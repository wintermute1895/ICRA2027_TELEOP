"""Tests for A7lite arm safety verification with hardware (interactive)."""
...
pytestmark = [pytest.mark.a7_lite, pytest.mark.interactive, pytest.mark.safety]
...
class TestZeroPointVerification:
    ...
    def test_manual_zero_check(
        ...
        session.step(
            ...
            action=lambda: print(
                f"\nCurrent angles: {a7_lite_arm.get_angles()}\n"
                f"All within tolerance: "
                f"{all(abs(a) < ZERO_TOLERANCE_RAD for a in a7_lite_arm.get_angles())}"
            ),
            expected=(
...
class TestJointDirectionVerification:
    ...
    def test_all_joint_directions(
        ...
        for i in range(NUM_JOINTS):
            ...
            target_direction = (
                "NEGATIVE" if i == 1 and arm_side == "left" else "POSITIVE"
            )
            target_angle_str = "- 0.2" if i == 1 and arm_side == "left" else "+ 0.2"
            ...
            session.step(
                ...
                action=lambda t=target_pos: (
                    a7_lite_arm.move_j(t, blocking=True),
                    a7_lite_arm.wait_motion_done(),
                    time.sleep(2),
                    a7_lite_arm.home(blocking=True),
                    a7_lite_arm.wait_motion_done(),
