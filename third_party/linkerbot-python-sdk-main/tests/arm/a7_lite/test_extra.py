"""Tests for A7lite arm PID setting and zero calibration (interactive).
...
from tests.arm.a7_lite.conftest import NUM_JOINTS, get_pid_params
from tests.conftest import InteractiveSession

pytestmark = [pytest.mark.a7_lite, pytest.mark.interactive, pytest.mark.extra]


class TestPIDSetting:
    """Test PID parameter read/write/restore cycle (non-destructive)."""

    def test_pid_read_write_restore(self, a7_lite_arm: A7lite) -> None:
        """Read PID -> modify each value +0.1 -> verify -> restore -> verify."""
        a7_lite_arm.enable()
        try:
            original = get_pid_params(a7_lite_arm)
            ...
            new_filt_gain = [v + 0.1 for v in original["speed_filt_gain"]]

            a7_lite_arm.set_position_kps(new_loc_kp)
            a7_lite_arm.set_velocity_kps(new_speed_kp)
            a7_lite_arm.set_velocity_kis(new_speed_ki)
            a7_lite_arm.set_velocity_filt_gains(new_filt_gain)

            updated = get_pid_params(a7_lite_arm)
            for key in original:
            ...
            a7_lite_arm.set_position_kps(original["loc_kp"])
            a7_lite_arm.set_velocity_kps(original["speed_kp"])
            a7_lite_arm.set_velocity_kis(original["speed_ki"])
            a7_lite_arm.set_velocity_filt_gains(original["speed_filt_gain"])

            restored = get_pid_params(a7_lite_arm)
            for key in original:
                for i in range(NUM_JOINTS):
...
class TestCalibrateZero:
    ...
    def test_calibrate_zero_flow(
        self, a7_lite_arm: A7lite, interactive_session: InteractiveSession
    ) -> None:
        ...
        session.step(
            ...
            action=lambda: (
                a7_lite_arm.enable(),
                a7_lite_arm.set_velocities([0.3] * NUM_JOINTS),
        ...
        session.step(
            ...
            action=lambda: a7_lite_arm.move_j(test_target, blocking=True),
