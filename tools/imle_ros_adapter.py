#!/usr/bin/env python3
"""ROS2 side of IMLE deployment; the model itself runs in the training env."""
...
def jpeg(msg: Image) -> bytes:
    ...
    if msg.step != msg.width * channels:
        raise ValueError("IMLE camera image has unsupported row padding")
...
class IMLEAdapter(Node):
    def __init__(self, config: dict) -> None:
        super().__init__("imle_ros_adapter")
        validate_runtime_config(config)
        self.timeout_s = float(config.get("input_timeout_ms", 300.0)) / 1000.0
        self.action_units = config.get("action_units", "degrees")
        ...
        self.inference_executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="imle-worker")
        ...
        self.create_timer(1.0 / float(config.get("inference_hz", 50.0)), self.infer)
    ...
    def exchange(self, request: dict, images: dict[str, Image]) -> dict:
        ...
        if not line:
            raise OSError("IMLE worker disconnected")
...
def main() -> int:
    ...
    if config.get("enabled") is not True:
        raise SystemExit("IMLE adapter is disabled in runtime config")
    rclpy.init()
    node = IMLEAdapter(config)
    try:
        rclpy.spin(node)
