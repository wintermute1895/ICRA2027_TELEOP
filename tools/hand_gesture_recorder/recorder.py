...
def main(argv=None):
    ...
    p.add_argument("--can", default="can0"); p.add_argument("--output", default="gestures"); p.add_argument("--output-stem", help="独立输出文件名（不含 .json/.py），用于保留已有手势文件"); p.add_argument("--step", type=float, default=1.0); p.add_argument("--simulate", action="store_true")
    a=p.parse_args(argv); device=create_adapter(a.hand,a.side,a.can,a.simulate); values=device.read(); selected=0; selected_joints={0}
    stem = a.output_stem or f"{a.hand.lower()}_gestures"
    existing = Path(a.output) / f"{stem}.json"
    if existing.exists():
        try:
        ...
        saved = GestureSet(a.hand)
    number = max(saved.gestures, default=-1) + 1
    status = f"Ready; next gesture number is {number}" if number else "Ready; next gesture number is 0"
    ...
    try:
        for key in _key_stream():
            ...
            if key in ("s","S"):
                ...
                _draw(device, values, selected, selected_joints, len(saved.gestures), f"Saved gesture {number - 1}"); continue
            if key in ("a", "A"):
        ...
        device.close()
    if saved.gestures: print("\nWritten:", *saved.export(a.output, stem=stem))
