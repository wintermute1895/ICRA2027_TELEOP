...
## Fixed reference trajectory replay

The reference replayer accepts the CSV exported by the data flywheel and
publishes only to a command namespace beginning with `/sim`. It rejects
`/robot1` and other non-simulation namespaces, so it cannot be used as a
hardware command source. It validates the expected seven right-arm joint
names, finite radian values, contiguous sample indices, and increasing times.
It first uses a simulation-only `MoveJ` command to bring the arm to the
recorded start pose at a limited `0.05 rad/s` speed and `0.05 rad/s^2`
acceleration, confirms the simulated joint state is within 0.01 rad, and only
then replays the fixed path.

On the MuJoCo host, rebuild after pulling the package update, then launch:

```bash
cd /mnt/F/ICRA2027_TELEOP/ros2_ws
source /opt/ros/humble/setup.bash
colcon build --packages-select sim_robot_driver --symlink-install
source install/setup.bash

ros2 launch sim_robot_driver replay_reference_trajectory.launch.py \
  trajectory_csv:=/mnt/F/ICRA2027_TELEOP/reports/a_insertion_reference_medoid_v1/reference_trajectory_low_speed_v1.csv \
  model_path:=/mnt/F/ICRA2027_TELEOP/assets/robots/linker_platform/sensorized/a7_l10_task_scene.mjcf.xml \
  render:=true
```

The replay runs once at its recorded 10 Hz timing and then holds the final
simulated joint command. It is an arm-motion and collision-validation replay;
the CSV does not contain gripper commands or an insertion-success detector.
