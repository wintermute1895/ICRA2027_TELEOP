#!/usr/bin/env python3
"""Assign a user-confirmed experimental condition to D0 episodes 100-139."""
...
BACKUP_ROOT = Path("reports/d0_first_condition_manifest_backup_20260826")
...
def main() -> int:
    ...
    for manifest in manifests:
        ...
        record["condition_label_source"] = "user"
        record["condition_labeled_utc"] = timestamp
        temporary = manifest.with_suffix(".json.tmp")
        temporary.write_text(json.dumps(record, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        temporary.replace(manifest)
    print(json.dumps({"assigned_condition": CONDITION_ID, "episode_count": len(manifests), "backup_root": str(BACKUP_ROOT)}, ensure_ascii=False))
    return 0
