...
set -u
if [[ ! -f "${ROOT_DIR}/arm_teleop/install/setup.bash" ]]; then
  echo "ROS2 workspace is not built. Run scripts/build_ros2.sh first." >&2; exit 2
fi
set +u
...
if (( ! REAL )); then
  ...
  python3 "${ROOT_DIR}/scripts/check_teleop_environment.py" --mode ros2 || true
  exit 0
fi
...
exec ros2 launch lbot_teleop teleop.launch.py armed:=true
