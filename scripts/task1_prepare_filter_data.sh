#!/usr/bin/env bash
# Prepare every Task1 (precision alignment) episode into a filter-training VLM view.
# Run inside the teleop conda environment:
#   conda activate teleop
#   bash scripts/task1_prepare_filter_data.sh
# Log is written automatically next to the model outputs under
# <run_root>/logs/ while still being printed to the terminal.
# Overrides: TASK1_DATA_DIR, TASK1_FLYWHEEL_CONFIG, TASK1_DERIVED_NAME,
#            TASK1_RUN_ROOT, TASK1_LOG_DIR
set -Eeuo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
DATA_DIR="${TASK1_DATA_DIR:-/media/fanshihao/Cyan_data/ICRA2027_DATA/Task_Data/Task1_Data}"
CONFIG="${TASK1_FLYWHEEL_CONFIG:-$ROOT_DIR/config/flywheel/task1_precision_alignment_local.yaml}"
...
export TMPDIR="${TASK1_TMPDIR:-${TMPDIR:-$RUN_ROOT/tmp}}"
mkdir -p "$TMPDIR"
