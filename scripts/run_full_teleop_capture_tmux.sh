#!/usr/bin/env bash
# One-command, tmux-based VIST real-robot teleoperation capture launcher.
# Safe by default: the teleop bridge is started with armed=false.
set -Eeuo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SESSION="vist_capture"
DURATION_S="30"
EPISODES="2"
REAL=0
ESTOP_READY=0
...
usage() {
  cat >&2 <<'EOF'
Usage:
  run_full_teleop_capture_tmux.sh [options]
...
Options:
  ...
  --session NAME                 tmux session name (default: vist_capture)
...
[[ -f "$ROOT_DIR/arm_teleop/install/setup.bash" ]] || die "ROS2 workspace is not built"
[[ -f "$ROOT_DIR/scripts/run_teleop_evidence_capture.sh" ]] || die "recorder script is missing"

set +u
source /opt/ros/humble/setup.bash
source "$ROOT_DIR/arm_teleop/install/setup.bash"
set -u
...
PROCESS_CHECKS=(
  ...
  "teleop_bridge_node|(^|/)teleop_bridge_node([[:space:]]|$)"
...
EXISTING_NODES="$(timeout 5s ros2 node list 2>/dev/null | grep -E '^/(camera/camera|linkerta_node|teleop_bridge_node|robot1/lbot_(main|left_arm|right_arm)_node)$' || true)"
...
python3 "$ROOT_DIR/scripts/check_teleop_environment.py" --mode ros2
...
ROBOT_IP="$(awk -F'"' '/slave_arm_ips:/{flag=1;next} flag && /- "/{print $2; exit}' "$ROOT_DIR/arm_teleop/src/lbot_teleop/config/teleop_config.yaml")"
[[ -n "$ROBOT_IP" ]] || die "cannot read robot IP from teleop_config.yaml"
...
CONFIG="$ROOT_DIR/arm_teleop/src/lbot_teleop/config/teleop_config.yaml"
grep -Eq 'enable_joint_limits:[[:space:]]*true' "$CONFIG" || die "joint limits are not enabled in teleop_config.yaml"
grep -Eq 'enable_one_euro_filter:[[:space:]]*true' "$CONFIG" || die "One-Euro filter is not enabled in teleop_config.yaml"
...
launch_cmd() {
  local title="$1"; shift
  tmux new-window -t "$SESSION" -n "$title" "bash -lc 'set +u; source /opt/ros/humble/setup.bash; source \"$ROOT_DIR/arm_teleop/install/setup.bash\"; set -u; export ROS_LOG_DIR=\"$ROS_LOG_DIR\"; $*; exec bash'"
}
...
tmux new-session -d -s "$SESSION" -n preflight "bash -lc 'echo VIST capture preflight passed; echo mode=$([[ $REAL -eq 1 ]] && echo REAL_ARMED || echo SAFE_OBSERVATION); echo robot_ip=$ROBOT_IP; echo camera=$CAMERA_SERIAL ${WIDTH}x${HEIGHT}@${FPS}; exec bash'"
...
launch_cmd teleop "ros2 launch lbot_teleop teleop.launch.py launch_driver:=false armed:=$ARMED_ARG"
wait_for_topic /left_arm_joint_control 20
wait_for_topic /right_arm_joint_control 20
wait_for_topic /vist/left/mapped_joint_command 20
wait_for_topic /vist/right/mapped_joint_command 20
if (( PREVIEW )); then
  if [[ -n "${DISPLAY:-}" ]]; then
...
launch_cmd sync "python3 \"$ROOT_DIR/tools/diagnose_ros_time_sync.py\" --duration-s 10 --camera-namespace /camera/camera --output \"$RUN_ROOT/pre_capture_time_sync.json\""

RECORDER="$ROOT_DIR/scripts/run_teleop_evidence_capture.sh"
launch_cmd recorder "for i in \$(seq 1 $EPISODES); do export TELEOP_CAPTURE_DURATION_S=$DURATION_S; export TELEOP_HARDWARE_COMMANDS_ENABLED=$([[ $REAL -eq 1 ]] && echo true || echo false); export RUNEVIDENCE_BAG_COMPRESSION_MODE=file; export RUNEVIDENCE_BAG_COMPRESSION_FORMAT=zstd; echo READY_EPISODE_\$i; read -r -p \"按回车开始 episode \$i（Ctrl-C 可中止）: \"; \"$RUNEVIDENCE_BIN\" run --domain robotics --runs-root \"$RUN_ROOT\" --label \"teleop-episode-\$i\" --input camera_serial=\"$CAMERA_SERIAL\" --input camera_profile=\"${WIDTH}x${HEIGHT}x${FPS}\" --input teleop_armed=\"$ARMED_ARG\" --input teleop_config=\"$CONFIG\" -- bash \"$RECORDER\"; done; echo ALL_EPISODES_COMPLETE; exec bash"
